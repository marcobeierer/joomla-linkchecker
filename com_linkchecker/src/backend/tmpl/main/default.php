...
<div class="bootstrap3" style="margin-top: 10px;">
		...
		<ul class="nav nav-tabs" role="tablist" style="margin-bottom: 20px;">
			...
			<?php foreach ($this->websiteURLs as $websiteURL) : ?>
				<li role="presentation" class="<?php echo $firstWebsite ? 'active' : ''; ?>">
					<a href="#<?php echo md5($websiteURL); ?>" aria-controls="<?php echo md5($websiteURL); ?>" role="tab" data-toggle="tab"><?php echo $this->escape($websiteURL); ?></a>
				</li>
	...
	<div class="tab-content">
		...
		<?php foreach ($this->websiteURLs as $websiteURL) : ?>
			<div
				role="tabpanel"
				class="tab-pane <?php echo $firstWebsite ? 'active' : ''; ?>"
				id="<?php echo md5($websiteURL); ?>"
			>
				<linkchecker
					id="<?php echo $count; ?>"
					website-url="<?php echo $this->escape($websiteURL); ?>"
					token="<?php echo $this->escape($this->token); ?>"
					origin-system="joomla"
					email="<?php echo $this->escape($this->email); ?>"
					max-fetchers="<?php echo (int) $this->maxFetchers; ?>"
					login-page-url="<?php echo $this->escape($this->loginPageURL); ?>"
					login-form-selector="<?php echo $this->escape($this->loginFormSelector); ?>"
					login-data="<?php echo $this->escape($this->loginData); ?>"
				>
				</linkchecker>
			</div>
			<?php
